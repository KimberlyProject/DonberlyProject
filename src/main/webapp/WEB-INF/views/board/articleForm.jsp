...
<%
	...
	if(session.getAttribute("isLogOn") == null) {
		...
		*/
		// http://localhost:8097/board/articleForm.do
		PrintWriter pw = response.getWriter();
		pw.println("<script>");
		pw.println("alert('먼저 로그인을 하셔야 글을 쓰실 수 있습니다!!!');");
		pw.println("location.href='/member/login?action=/board/articleForm.do';");
		pw.println("</script>");
		pw.flush();
		pw.close();
...
<div class="container">
	...
	<form name="articleForm" method="post" action="${path}/board/addNewArticle.do" enctype="multipart/form-data">
		<table class="table">
			<tr>
...
function backToList(obj) { // 게시글 목록 페이지로 이동한다.
	location.href = "${path}/board/listArticles.do";
